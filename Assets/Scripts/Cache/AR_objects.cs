...
public class AR_objects : MonoBehaviour
{
    ...
    protected static string GetUrl(int id) {
        ...
        if (url != null)
        {
            return url.Contains("http") ? url : GlobalVariables.link + url;
        }
        return "";
        #endregion
    }
...
{ 
    public IEnumerator DownloadAndCache(int id)
    {
        ...
        {
            cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Marker, i.ToString());            
...
{  
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Logo);       
...
{   
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Sticker);       
        folder = PathWorker.MarkerStrPath(id);
        filename = "sticker";
...
{    
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Image);       
...
public class AR_video : AR_objects, ICacheble
{
    ...
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Video);       
        folder = PathWorker.ArObjStrPath(id);
        filename = GetFileName(id);
        url = GetUrl(id);
...
public class AR_audio : AR_objects, ICacheble
{
    ...
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Audio);       
        folder = PathWorker.ArObjStrPath(id);
        filename = GetFileName(id);
        url = GetUrl(id);
...
{  
    public IEnumerator DownloadAndCache(int id)
    {
        cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.AssetBundle); 
        folder = PathWorker.ArObjStrPath(id);
        filename = GetFileName(id);
        url = GetUrl(id);
        yield return StartCoroutine(StartDownloadAndCache(cacheFilePath, folder, filename, url));
...
public class AR_text : AR_objects, ICacheble
{
    public IEnumerator DownloadAndCache(int id)
    {
        var cacheFilePath = PathWorker.CacheFilePath(id, ArObjects.Type.Text);
