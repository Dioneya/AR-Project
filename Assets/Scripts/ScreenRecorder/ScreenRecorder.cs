...
namespace Recorder
{
    public class ScreenRecorder : MonoBehaviour
    {
        ...
        private const string VIDEO_NAME = "Record", GALLERY_PATH = "/../../../../ARProject/VideoRecords";
    ...
    private AndroidJavaObject androidRecorder;
        ...
        private void Start()
        {
            GetPermissions(AndroidPermission.WRITE_EXTERNAL_STORAGE, "Нужен доступ 'Запись на устройство' для сохранения фото и видео", "Нужен доступ 'Запись на устройство' для сохранения фото и видео");
            GetPermissions(AndroidPermission.RECORD_AUDIO, "Нужен доступ к микрофону для записи видео", "Нужен доступ к микрофону для записи видео");
            //GetPermissions(AndroidPermission.ACCESS_COARSE_LOCATION, "Нужен доступ к вашему текущему местоположению (для определения ближайщих заведений и квестов)", "Нужен доступ к вашему текущему местоположению (для определения ближайщих заведений и квестов)");
            //GetPermissions(AndroidPermission.ACCESS_FINE_LOCATION, "Нужен доступ к вашему текущему местоположению (для определения ближайщих заведений и квестов)", "Нужен доступ к вашему текущему местоположению (для определения ближайщих заведений и квестов)");
        ...
        private void GetPermissions(AndroidPermission permission, string onDenyCallbackText, string onDenyAndNeverAskAgainCallbackText) {
            if (!ScreenRecorder.IsPermitted(permission))//request this permission to write recorded file to disk
            {
                ScreenRecorder.RequestPermission(permission);
                onDenyCallback = () => { ShowToast(onDenyCallbackText); };
                onDenyAndNeverAskAgainCallback = () => { ShowToast(onDenyAndNeverAskAgainCallbackText); };
            }
        }
